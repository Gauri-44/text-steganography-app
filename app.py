...
def main():
    ...
    if app_mode == "Encode":
        ...
        if st.button("Encode"):
            if not uploaded_file or not secret_text.strip() or not secret_key.strip():
                st.warning("Please fill in all fields!")
            else:
                try:
                    file_content = uploaded_file.read().decode("utf-8")
                    sanitized_text = secret_text.strip()
                    encoded_data = encode_text(file_content, sanitized_text, secret_key.strip())
        ...
        if st.button("Decode"):
            if not uploaded_file or not secret_key.strip():
                st.warning("Please fill in all fields!")
            else:
                try:
                    file_content = uploaded_file.read().decode("utf-8")
                    secret_text = decode_text(file_content, secret_key.strip())
